...
let MicroManage = class MicroManage {
    ...
    bootstrapMicro(microName) {
        ...
        if (!subject) {
            ...
            subject = this.http.get(`${this.proxy}${microPath}`).pipe((0, import_rxjs_2.catchError)((error) => (0, import_rxjs_1.of)({ html: `${microName}<br/>${error.message}`, styles: '' })), (0, import_rxjs_2.switchMap)((microResult) => this.reeadLinkToStyles(microName, microResult)), (0, import_rxjs_2.map)((microResult) => ({ microResult: this.createMicroTag(microName, microResult), microName })), (0, import_rxjs_2.shareReplay)(1));
    ...
    reeadLinkToStyles(microName, microResult) {
        ...
        return (0, lodash_1.isEmpty)(links) ? (0, import_rxjs_1.of)(microResult) : (0, import_rxjs_1.forkJoin)(links.map((href) => this.getLinkCache(`${this.proxy}${href}`))).pipe((0, import_rxjs_2.map)((styles) => ({ ...microResult, linkToStyles: styles })));
    }
    getLinkCache(href) {
        let linkSubject = this.microStaticCache.get(href);
        if (!linkSubject) {
            linkSubject = this.http.getText(href).pipe((0, import_rxjs_2.shareReplay)(1), (0, import_rxjs_2.map)(lodash_1.cloneDeep));

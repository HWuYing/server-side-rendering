...
let MicroManage = class MicroManage {
    ...
    bootstrapMicro(microName) {
        ...
        if (!subject) {
            ...
            subject = this.http.get(`${this.proxy}${microPath}`).pipe((0, import_rxjs_1.catchError)((error) => (0, import_rxjs_1.of)({ html: `${microName}<br/>${error.message}`, styles: '' })), (0, import_rxjs_1.switchMap)((microResult) => this.reeadLinkToStyles(microName, microResult)), (0, import_rxjs_1.map)((microResult) => ({ microResult: this.createMicroTag(microName, microResult), microName })), (0, import_rxjs_1.shareReplay)(1));
            subject.subscribe(() => void (0), () => void (0));
            this.appContext.registryMicroMidder(() => subject);
            this.microCache.set(microName, subject);
        }
        return (0, import_rxjs_1.of)(null);
    }
    reeadLinkToStyles(microName, microResult) {
        const { links = [] } = microResult;
        return (0, lodash_1.isEmpty)(links) ? (0, import_rxjs_1.of)(microResult) : (0, import_rxjs_1.forkJoin)(links.map((href) => this.getLinkCache(`${this.proxy}${href}`))).pipe((0, import_rxjs_1.map)((styles) => ({ ...microResult, linkToStyles: styles })));
    }
    getLinkCache(href) {
        let linkSubject = this.microStaticCache.get(href);
        if (!linkSubject) {
            linkSubject = this.http.getText(href).pipe((0, import_rxjs_1.shareReplay)(1), (0, import_rxjs_1.map)(lodash_1.cloneDeep));
